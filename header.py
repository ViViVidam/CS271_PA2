...
class GlobalState:
    ...
    def setState(self,id,deposit,channelMessages):
        self.clientStates[id].deposit = deposit
        self.clientStates[id].channelMessages = channelMessages
...
def submit_states(stateIdentifier,id,deposit,channelMessages):
    try:
        index = globalSnapshots.index(stateIdentifier)
    except:
        print("{} not int the {}".format(stateIdentifier,globalSnapshots),flush=True)
    globalStates[index].setState(id,deposit,channelMessages)